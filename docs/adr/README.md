...
| [ADR_20260924-2_core_sync-writes-references](ADR_20260924-2_core_sync-writes-references.md) | sync writes `references` as relations like the other edge kinds; the type is created `visibility: hidden`, a view switches them on; supersedes ADR_20260923-9 §5 on `references` and the rejected `byType` of ADR_20260828 |
