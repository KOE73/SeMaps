...
| Тип | Статус | Число |
|---|---|---|
| `contains` | present | 770 |
| `extends` | present | 118 |
| `implements` | present | 107 |
| `depends` | present | 6 |
| `injects` | present | 177 |
| `holds.*` открытые | present | 71 |
| `holds.*.internal` | present | 34 |
| `references` (старая модель) | missing | 643 |

Первый прогон оставил старые `references` присутствующими: сверка пропускала незнакомые типы.
Исправлено (`3e8033f`), после повторной сверки все 643 — `missing`, как в §8 ADR.
