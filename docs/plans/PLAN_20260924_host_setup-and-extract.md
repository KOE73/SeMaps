...
Статус: **закрыт** — сделаны все шаги.
...
2. ~~**Поиск экстракторов и `semaps doctor`.** Порядок: `command` → `extractors/<lang>/`
   ...
   экстрактору из `.semaps` нечем запуститься.~~ `host/tools.go`, `host/commands.go`.
3. ~~**`semaps extract [id]` и `semaps sync [id] [--dry-run]` без `--facts`.** Прогон — папка
   ...
   интерфейсом `runner`. `--facts` остаётся.~~ `host/runs.go`. Сделано иначе, чем
   задумано: `sync` без `--run` всегда извлекает заново (не берёт старый прогон
   молча); `--run <id>` — явно прогон; выбор экстрактора — `--extractor <id>`.
4. ~~**Поставка.** `install.cmd` и CI: `dotnet publish` экстрактора C# и бандл экстрактора TS
   ...
   README; `extractors/csharp/README.md` — «запуск отдельно» остаётся.~~ Релиз — zip на
   платформу (`semaps` + `extractors/`); голые бинарники остаются.
5. ~~**HTTP API.** `GET /api/tools`, `GET`/`PUT /api/setup` (без `command`), `POST /api/runs`,
   ...
   `host/api_tools.go`, `API.md` §5. Лог — опросом по смещению, не SSE: проще и проходит
   любой прокси.
6. ~~**Страницы инструмента.** Уточняет ADR §4 по раскладке:
   ...
   - **общая верхняя полоса** на всех страницах (редактор, `/setup`, `/extract`): имя проекта,
     ...
     общий компонент, потом строить страницы на нём.~~ `editor/src/shell/`,
   `editor/setup.html`, `editor/extract.html`. Полоса встаёт над лентой в `app/main.ts`.
   Кнопки «Сверить» в ленте нет: туда ведёт «Экстракторы» полосы; редактор, открытый
   после записи, читает проект заново.
